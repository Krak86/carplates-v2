...
function BrandsInfo(): ReactNode {
  const { t } = useTranslation()
  return (
    <div className="flex flex-col gap-2">
      <p>{t('safety.brandsInfoIntro')}</p>
      <div>
 ...
 * have a match. Fetched only once expanded, mirroring VehiclePhotos.
 */
export default function SafetyRatings({ brand, model, year }: Props): ReactNode {
  ...
  const result = useQuery({ ...safetyRatingsQuery(brand ?? '', model ?? '', year ?? 0), enabled: open && hasQuery })
